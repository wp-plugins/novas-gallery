...
function NGAdminEnqueue($hook) {
	...
	wp_localize_script('NGAdmin', 'ajax_object', array( 'ajax_url' => admin_url('admin-ajax.php')));
	wp_enqueue_media();
}
...
add_action('wp_ajax_NGAdd', 'NGAdd');
function NGAdd() {
	global $wpdb;
	$table_name = $wpdb->prefix.'NGLocal';
	$data = array (
...
add_action('wp_ajax_NGRemove', 'NGRemove');
function NGRemove() {
	global $wpdb;
	$table_name = $wpdb->prefix.'NGLocal';
	$where = array (
...
add_action('wp_ajax_NGNew', 'NGNew');
function NGNew() {
	global $wpdb;
	$table_name = $wpdb->prefix.'NGFeeds';
	$data = array (
	...
	$wpdb->insert($table_name, $data);
	echo '<id>'.$wpdb->insert_id.'</id>';
	echo '<types>'.get_types('-').'</types>';
	die();
}
add_action('wp_ajax_NGDelete', 'NGDelete');
function NGDelete() {
	global $wpdb;
	$table_name = $wpdb->prefix.'NGFeeds';
	$where = array (
...
add_action('wp_ajax_NGSave', 'NGSave');
function NGSave() {
	global $wpdb;
	$table_name = $wpdb->prefix.'NGFeeds';
	$data = array (
